...
scores = evaluate_modelhRF(X, y, 5, 100)
# summarize
...
# print("Best score of middle value", bestscoresls[middleval])
# print(bestmodelsls[getidx].coef_)  # Print the coeficients of the best model
# # Print the regression results
# regression_results(y_predicted, y_obs)
# figf, axf = plt.subplots()
# lims = [
#     np.min([y_predicted.transpose(), y_obs.transpose()]),  # min of both axes
#     np.max([y_predicted.transpose(), y_obs.transpose()]),  # max of both axes
# ]
# axf.plot(lims, lims, marker="o", alpha=0.5)
# axf.hist2d(y_obs.transpose(), y_predicted.transpose(), bins=50, cmap='YlGn')
#
...
shap_values = shap.TreeExplainer(bestmodel).shap_values(X_train)
...
plt.title("Feature importance of whole dataset")
plt.show()
...
for df in listdfs:
    ...
    length = len(allfeats)
    for col in cleandfs[dfname].columns.values[length:]:  # 13 to 14 because I added the K_mean_marshType col for visualuization
...
for key in cleandfs:
    ...
    #plot
    ABS_SHAP(shap_values, X_train)
    plt.title("Feature importance of " + str(key))
    plt.show()
...
for df in listdfscontrols:
    ...
    cleandfscontrols[dfname] = df[allfeatsSplits].dropna()  # extract the most important features identified for that dataframe
    ...
    length = len(allfeats)
    for col in cleandfscontrols[dfname].columns.values[length:]:  # 13 to 14 because I added the K_mean_marshType col for visualuization

        cleandfscontrols[dfname] = cleandfscontrols[dfname][np.abs(cleandfscontrols[dfname][col]) < 3]  # keep if value is less than 2 std
